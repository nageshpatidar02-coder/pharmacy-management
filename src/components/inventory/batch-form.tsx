...
export function BatchForm({ medicines }: { medicines: MedicineOption[] }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); setSaving(true); setError(""); const data = Object.fromEntries(new FormData(event.currentTarget)); try { const response = await fetch("/api/batches", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) }); const result = await response.json(); if (!response.ok) { setError(result.error ?? "Unable to add batch."); return; } event.currentTarget.reset(); router.refresh(); } catch { setError("Inventory service is unavailable."); } finally { setSaving(false); } }
