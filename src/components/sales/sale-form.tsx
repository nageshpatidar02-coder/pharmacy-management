...
type Medicine = { id: string; name: string; sellingPrice: number; batches: Batch[] };
type Line = { medicineId: string; batchId: string; quantity: number; sellingPrice: number; discount: number };
const emptyLine = (): Line => ({ medicineId: "", batchId: "", quantity: 1, sellingPrice: 0, discount: 0 });
...
export function SaleForm({ customers, medicines }: { customers: Customer[]; medicines: Medicine[] }) {
	...
	function selectMedicine(index: number, medicineId: string) { const medicine = medicines.find((entry) => entry.id === medicineId); const batch = medicine?.batches[0]; updateLine(index, { medicineId, batchId: batch?.id ?? "", sellingPrice: batch?.sellingPrice ?? medicine?.sellingPrice ?? 0 }); }
	...
	async function submit(event: React.FormEvent<HTMLFormElement>) {
		event.preventDefault(); setSaving(true); setError(""); const parsed = saleSchema.safeParse({ customerId, invoiceNumber, paidAmount, paymentMethod, items: lines });
