...
export async function GET() { const user = await requirePermission(PERMISSIONS.salesView); return NextResponse.json(await prisma.sale.findMany({ where: { pharmacyId: user.pharmacyId }, include: { customer: true, items: true }, orderBy: { invoiceDate: "desc" } })); }
