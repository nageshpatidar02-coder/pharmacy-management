...
export async function POST(request: Request) { try { const user = await requirePermission(PERMISSIONS.medicineCreate); const medicine = await createMedicine(await request.json()); return NextResponse.json({ medicine, createdBy: user.id }, { status: 201 }); } catch (error) { console.error(error); const message = error instanceof Error ? error.message : "Unable to create medicine."; return NextResponse.json({ error: message }, { status: message.includes("already in use") ? 409 : 400 }); } }
